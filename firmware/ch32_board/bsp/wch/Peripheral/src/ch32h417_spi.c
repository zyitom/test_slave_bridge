...
void SPI_I2S_DeInit(SPI_TypeDef *SPIx)
{
    ...
    {
        RCC_HB1PeriphResetCmd(RCC_HB1Periph_SPI2, ENABLE);
        RCC_HB1PeriphResetCmd(RCC_HB1Periph_SPI2, DISABLE);
    }
    else if(SPIx == SPI3)
    {
        RCC_HB1PeriphResetCmd(RCC_HB1Periph_SPI3, ENABLE);
        RCC_HB1PeriphResetCmd(RCC_HB1Periph_SPI3, DISABLE);
    }
    else if(SPIx == SPI4)
    {
        RCC_HB1PeriphResetCmd(RCC_HB1Periph_SPI4, ENABLE);
        RCC_HB1PeriphResetCmd(RCC_HB1Periph_SPI4, DISABLE);
    }
}
 ...
 *
 * @brief   Enables or disables the specified SPI high speed mode.
 *
 * @param   SPIx - where x can be
 *            - 1, 2 , 3 or 4 in SPI mode.
 *          SPI_SPEED_MODE - specifies the SPI/I2S interrupt source to be
 *        enabled or disabled.
 *            SPI_HIGH_SPEED_MODE1 - SPI high speed mode1.
 *            SPI_HIGH_SPEED_MODE2 - SPI high speed mode2.
 *          NewState - ENABLE or DISABLE.
 *
 * @return  none
 */
...
{
    SPIx->HSCR &= ~(SPI_HSCR_HSRXEN | SPI_HSCR_HSRXEN2);

    if(NewState != DISABLE)
    {
        SPIx->HSCR |= SPI_SPEED_MODE;
    }
}