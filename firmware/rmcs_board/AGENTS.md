...
> **状态**：历史记录。缺口本身已由下一节的 EP0 配置通道填上，但下面这段"为什么
...
### 通道配置走 EP0，不再走数据流 [实测 2026-09-04]

**结论先行：UART 波特率与 CAN 帧类型（`is_fdcan`）不再出现在 bulk 数据流里，改由
EP0 vendor 请求在主机侧构造板对象时下发并回读校验。板端拒绝 = STALL status stage，
主机侧 = 抛异常。**

协议定义在 `core/include/librmcs/protocol/vendor_control.hpp`（主机与固件共用），
板端实现在 `app/src/usb/vendor_control.cpp`，主机端封装在
`host/include/librmcs/board/rmcs_config.hpp`。
...
1. **`kSetCanConfig` 不会重配控制器。** `mcan_init()` 那一整块（87.5% 采样点、TDC、
   ...
   "主机的预期和我一致吗"这一件事。**推论：`CanDataView::is_fdcan` 在 rmcs_board 上
...
2. **回读永远不等于请求值。** `effective_baudrate()` 由实际写进去的分频器反推，
   ...
   几乎所有 80 MHz 除不尽的速率上误报失败（本次开发中真的踩了一次）。
...
`Timed out waiting for SESSION_ACK`。这挡的是**旧版 SDK 连新固件**：它会继续逐帧设
`is_fdcan`，而固件已经不读那一位，于是拿到 FD 帧却以为是 classic，没有任何一层会报错。

两个实现要点，都是踩出来的：

1. **主机侧握手必须早于第一个 kStart。** `Handler` 构造是**同步等 SESSION_ACK** 的，
   而板类原本在成员初始化列表里 `handler_(...)` 之后才做 EP0 握手——那样板子永远等不到
   握手、主机永远等不到 ack。现在 `Handler` 多了一个 `BeforeSession` 钩子，在传输已建立、
   session 未开的窗口里跑；`Impl::start()` 也因此从构造函数里拆了出来。
...
2. **板端要在 session 结束时忘掉握手，主机要在重连时重做。** `tud_mount_cb` 只在**重新
...
**EtherCAT 桥是唯一例外**：它的传输是过程数据流，没有控制端点，所以
`LIBRMCS_APP_RELEASE_CORE1` 构建里带内 `kUart*Config` 通道保留原样。USB 构建里该
回调返回 `false`（进 discard 模式），即明确拒绝而不是静默丢弃。

**EP0 还提供 CAN 控制器状态查询**（`0x45 kGetCanStatus`，wIndex = 丝印编号）：回读
`TEC`/`REC`/`PSR.LEC`/`PSR.DLEC`/`TXBTO`/`TXBCF`/转发帧数。**这条在正式镜像上就能用**，
不需要 `-DLIBRMCS_CAN_DIAG=ON`，也不占用 `kUart0` 上行通道。判据：
...
验证工具：`host/examples/ep0_config_test.cpp`（一块 0xa902 板，不需要 CAN/UART 接线）。
...
`usb_packet_rate split3 0 10`，单管道（当时用 `LIBRMCS_SPLIT_CAN_ENDPOINT=0` 关掉第二对
端点，这样全部 57.7k 包都经过被仪表化的那条管道；今天单管道已是唯一形态），三轮：
...
> **状态**：历史记录。第二对 bulk 端点已于 2026-09-05 整体拆除（描述符、编译期宏
> `LIBRMCS_SPLIT_CAN_ENDPOINT`、EP0 的 `0x46 kSetEndpointMode`、主机侧
> `enable_priority_can_channel` 全部删除），现在只有一对 `0x01`/`0x81`。本节的机制推理
> 仍然成立，取舍结论已被「分端点已默认关闭」一节取代。
...
（当时的 `LIBRMCS_COPY_THEN_ARM`，vendor.cpp），turnaround 2.22 -> 1.27 us，**包率一模一样**：
...
> **以下三节（本节、「分端点的真实代价」、「分端点是个交叉点」）的结论已于
> 2026-09-05 被复测推翻，状态改为历史记录。** 它们描述的机制、测量方法和那三个坑仍然
> 成立，被推翻的只是"分端点在高 UART 负载下赢"这个取舍结论——2026-09-05 在同一个双板
> rig、同一个 `uartcontend` 上交替复测，**分端点在 0 / 82 / 92 kB/s 三个负载点、绑核与
> 不绑核两种主机状态下，每一个分位都输或打平**。新结论见下面「分端点已默认关闭」一节。
...
**当时本项目的选择：开（默认 1）**，因为这块板的 UART0 持续跑数据，工作点被认为在交叉点
右侧。**这个选择 2026-09-05 被推翻**：复测发现交叉点本身不复现（旧数据的负载轴是错的），
第二对端点已整体拆除，现在没有开关可切。
...
### 分端点已默认关闭：2026-09-05 复测它在每个分位都输 [实测 2026-09-05]

**结论先行：`AdvancedOptions::enable_priority_can_channel` 默认改为 `false`。上面三节
（2026-08-07）得出的"UART 超过约 10 kB/s 就该分端点"不再复现——同一个 rig、同一个
`uartcontend`、交替跑，分端点在 p50 / p90 / p99 上全输，p99.9 打平，同时还要付 22% 的
峰值包率。这个开关现在没有任何一个已知工作点值得打开。**

`dual_board_test uartcontend`，两块 HPM5321，每档交替 2~3 轮，轮内离散度 ±0.5 us：

| UART 负载 | 不分端点 p50 | 分端点 p50 | 不分端点 p99 | 分端点 p99 |
|---|---|---|---|---|
| 0（空载） | **98.9** | 105.2 | **107~108** | 127 |
| 82 kB/s（本板工作点） | **100.8~100.9** | 118.3~118.7 | **137.0~137.6** | 143.0~144.3 |
| 92 kB/s（线速） | **103.5** | 118.6~118.7 | 146.8 | 145.6~146.6 |

包率（`uartonly` 形态的 flood，各 3 轮）：**不分端点 63.3~63.6k，分端点 49.1~49.3k**。

**绑核不是原因。** 起初怀疑 2026-08-07 那批数据早于主机 IO 线程绑核（`set_io_thread_affinity`），
分端点当年补的是主机调度抖动。实测否掉：完全不绑核、不提 SCHED_FIFO 重跑 82 kB/s，
排序不变（不分端点 p50 101.5~101.8 / p99 141~144，分端点 p50 118.0~118.5 / p99 146.7~147.2），
只有 p99.9 两边都退到 600~1000 us 且互有胜负。`[实测]`

**真正的原因是负载轴对不上。** 2026-08-07 全部用的是 `uartcontend ... 2 4 6 8`，而
**本机只有 CPU 0~7，`8` 不存在**——`sched_setaffinity` 返回 EINVAL，负载线程保持游离。
今天复刻这条命令：

```
load_core=8:  offered= 30.1 kB/s (asked 92)   <- 只送出三分之一
load_core=5:  offered=119.8 kB/s (asked 92)   <- 反而超过 921600 的线速
```

不绑核那次两条曲线与空载对照逐位重合（不分端点 99.0 / 103.4 / 104.6，空载
98.9 / 103.3 / 107）。**所以旧表里标着"92 kB/s"的那一行，实际负载并不是 92。**
跨 session 比那张表从一开始就不成立，只有同一批交替跑的数据可比。这条同时是
`uartcontend` 的使用纪律：**先看 `offered=`，再看分位数**。

**机制上分端点一直是对的，只是永远补不回本。** 按实际送达量重排今天的数据：

| 实际送达 | 不分端点 p50 / p99 | 分端点 p50 / p99 |
|---|---|---|
| 0 | 98.9 / 107 | 105.2 / 127 |
| 26 kB/s | 99.3 / 134.4 | 105.7 / 138.4 |
| 88 kB/s | 100.7 / 125.7 | 117.4 / 144.8 |
| 120 kB/s | 103.4 / 146 | 118.8 / 147 |

分端点确实削平了负载敏感度——p99 在整个区间只涨 20 us（127->147），不分端点涨 39 us
（107->146），"削掉约一半"没说错。**但它从一个高 22 us 的起点出发**，而这个起点在 p50
上是恒定支出（空载 +6 us，带载 +17 us ≈ 一个主机服务间隔）。**斜率上省的填不平截距上
欠的**，所以交叉点不存在。这与上一节的二值性自洽：第二对 IN 端点只要存在就被持续轮询，
那笔开销与它有没有数据无关，所以它是**截距**而不是斜率，而截距赢不了斜率。

**那 17.5 us 的 p50 代价 ≈ 正好一个主机服务间隔（15.6 us）。** 2026-08-07 那节把这件事
写在 p99 上（"第二个端点的轮询偶尔把 CAN 端点的服务推迟一个槽"），复测显示它不是"偶尔"，
而是**中位数**。机制不变，量级被低估了。

#### 代价是二值的，不是按 URB 数量算的 [实测 2026-09-05]

上面三节把代价归给"第二对端点的 IN 侧挂着 **16 个** URB"。**"16"这个数与代价无关**：

| CAN 管道上挂着的 IN URB 数 | packets/s |
|---|---|
| 0 | **63.1~63.6k**（= 完全不分端点） |
| 1 | 49.2k |
| 2 / 4 / 8 / 16 | 49.2k |

**一个 URB 和十六个一模一样贵。** 控制器只要看到环非空就持续对该端点发 IN 令牌，环上排了
几个 TRB 不影响轮询节奏；NAK 在 usbmon 里不可见，所以这一层只能靠包率反推。推论：**没有
"把池调浅一点"这种折中**，只有"这条管道收不收"两种状态。

> **踩坑**：第一次测出"depth=0 也是 49.3k"，据此一度断言代价与 URB 无关、在板端。那是
> 因为测量工具是**静态链接**的，跑的还是改动前的库（旧版把 0 当非法值拒掉、回落成 16）。
> 重新链接后 depth=0 恢复到 63.1k。**改了静态库就必须重链所有独立编译的工具**。

#### 由此排除的一个设计：只分下行

OUT 端点不会被空轮询（主机没数据就不发 OUT 令牌），所以"CAN 下行独占第二对端点、上行回到
共享管道"在包率上是**免费**的——板端主循环本来就无条件 arm 并读取管道 1 的 OUT，
当时的 `kSetEndpointMode`（随第二对端点一起删除）只决定上行走哪条，所以这个模式不需要改固件。实测功能正常
（`stress 3000` 双向 0% 丢失），但**延迟与完全不分端点无法区分**：82 kB/s 下 p50 100.8、
p99 137.5~137.7，对照不分端点的 100.8 / 137.0~137.6。**下行那一侧本来就没有可削的
head-of-line 代价**，所以这条路不值得实现。`[实测 2026-09-05]`

#### 双峰分布的实际形状与它值多少 [实测 2026-09-05]

分位数分不清"单峰长尾"和"双峰"，而这个区别决定了该去修**频率**还是修**幅度**。
`dual_board_test` 现在支持 `RMCS_LATENCY_HIST=1`，按 2 us 分桶打印分布（桶宽远小于
15.6 us 的主机服务间隔）。交付配置、UART `delivered=80.9 kB/s`、10000 样本 0 超时：

| 区间 | 占比 | 说明 |
|---|---|---|
| 94~104 us（峰 96） | ~68% | 主峰 |
| 106~116 us | ~9.5% | 谷 |
| **118~124 us（峰 122）** | **~13%** | **次峰** |
| >126 us | ~1% | 尾 |

**两峰间距 26 us。空载时次峰完全消失**（106 us 以上合计不到 0.5%，单峰），所以次峰
100% 是 UART 撞出来的，不是总线量化或板端抖动。

**由此可以给这个问题定价：期望代价 = 13% x 26 us = 3.4 us。** 而分端点把它换成
100% x 17.5 us 的恒定支出——**修法比问题本身贵 5 倍**，这是"截距赢不了斜率"的另一种
算法，也是交叉点不存在的最直接证明。

**分端点也买不到确定性。** 有一种说法是"恒定的 118 比 96/122 双峰更好调"，实测不成立：
分端点 p50->p99 跨度 27.4 us（117.4->144.8），不分端点 26.2 us（100.7->126.9），
**跨度一样，只是整体抬高 17 us**。

**能动的两个杠杆**：
1. **降频率**——撞击概率正比于 UART chunk 率。IMU 类负载的 chunk 率由它自己的帧率钉死
   （1 kHz IMU 就是约 1000 chunk/s），要减半只能把 2 帧合成 1 个 USB 包，代价是 +1 ms
   的串口延迟。对 1 kHz 控制环通常不划算。
2. **降幅度**——那 26 us 是"错过当前传输窗口、等下一个"。板子每个端点同时只能挂 1 个
   qTD（TinyUSB 的选择，硬件支持 8，见 `hpm_soc_feature.h`），所以 UART 批次在飞时
   CAN 批次排不进去。**multi-qTD 是唯一能攻幅度的结构性改法**；此前否掉它的实验测的是
   饱和包率下的 device turnaround，**没有测过它对这个 26 us 有没有用**，属于未验证方向。
...
## EP0 上还剩什么，以及为什么留 [2026-09-05]

分端点拆除后 `0x46 kSetEndpointMode` 已删除，**编号空出不复用**——老主机来问会拿到
STALL，而不是被当成某个后加的请求重新解释。其余六条都不是为分端点存在的：

| bRequest | 留 | 理由 |
|---|---|---|
| `0x40 kGetInterface` | 是 | 现在是**承重的**：主机侧 5321 合并成一个类靠它报 `can_count`（见下一节） |
| `0x41/0x42` CAN 模式 | 是 | 构造期校验；`is_fdcan` 已被固件逐帧忽略，这是唯一的核对点 |
| `0x43/0x44` UART 波特率 | 是 | 求解失败能 STALL，带内通道表达不了失败 |
| `0x45 kGetCanStatus` | 是 | 正式镜像上读控制器错误寄存器，不用开 `CAN_DIAG`、不占 `kUart0` |
| `0x46` | **删** | 随分端点一起 |
| `0x47 kGetLatencyBreakdown` | 是 | 埋点 2026-09-05 起恒开，见下 |

### 会话门控保留，但它现在是第二道防线

"没做 EP0 握手就不给 session"挡的是**旧 SDK 连新固件**（旧 SDK 继续逐帧设 `is_fdcan`，
而固件已不读那一位，于是拿到 FD 帧却以为是 classic）。这个风险与分端点无关，没有变。

但要说清它的位置：`DeviceScanner::match_product_version` 比的是固件产品串里的版本号与
SDK 编译进去的 `LIBRMCS_PROJECT_VERSION_STRING`，**正常路径下旧 SDK 已经被挡住了**。
所以门控只在 `dangerously_skip_version_checks=true` 时才真正生效——而仓库自己的全部测量
工具都开着它，正是最容易混用镜像的那一群。成本是固件里约 5 行加一个虚函数；
`BeforeSession` 钩子不算它的成本，EP0 配置本来就要在 session 之前下发。

### 延迟拆解埋点：至多 0.5% 包率，恒开（开关 2026-09-05 删除）

`diag/latency.hpp` 的四个时间戳此前是**无条件编译**的，其中两个落在 `tud_vendor_rx_cb`
里——正是本文档测出"任何加进 rx_cb 的工作按 1.2~1.4 倍换算成包率损失"的那条路径。实测
（同一块板，交替刷 3 轮）：

| | packets/s |
|---|---|
| `LIBRMCS_LATENCY_PROBE=ON` | 63223 / 63577 / 63200 |
| `LIBRMCS_LATENCY_PROBE=OFF` | 63650 / 63629 / 63461 |

**约 0.4%，三对全部同向，但区间有重叠**——所以这是"至多约 0.5%"，不是一个精确值。
代价落在够不到的余量上（CAN 线速 19.8k f/s），而替代方案 `-DLIBRMCS_CAN_DIAG=ON`
会污染 `kUart0` 上行流、让 `dual_board_test uart` 必然 FAIL（2026-09-04 为此烧了六次
板、追了一个不存在的 UART 回归）——所以 **2026-09-05 起埋点恒开，`LIBRMCS_LATENCY_PROBE`
开关已删除**：EP0 的 `kGetLatencyBreakdown` 在 host 侧有消费者，OFF 那条路没有。上表
是这个决定的依据，仍然成立；真要重测就从 git 历史取回那个 `#if`。

### 什么放 EP0、什么放 bulk：判据 [2026-09-05]

反复被问到，写死在这里：

> **每帧的、周期性的、要和数据定序的 → EP1 bulk。构造期问一次的、失败必须让主机看见的
> → EP0。**

按这条判，现在两边的东西都在正确的位置：

| | EP0（控制传输，64 B，三阶段，主机发起） | EP1 bulk（`0x01`/`0x81`，512 B） |
|---|---|---|
| 内容 | `kGetInterface`、CAN 模式、UART 波特率、`kGetCanStatus`、`kGetLatencyBreakdown` | `kSession`、`kTimeAnchor`/`kTimeStatus`、CAN 帧、UART 字节 |
| 性质 | 一次性、能用 STALL 表达拒绝 | 周期性、带内定序、能证明数据面活着 |

**`kSession` 不搬 EP0**，三条理由，按硬度排：

1. **keepalive 要证明的是数据管道活着。** EP0 健康不等于 bulk 健康——本板出现过 bulk OUT
   在总线复位后没重新 arm、发得出收不到、而 EP0 一路正常应答的故障。session 建立在 EP0 上
   会把那种板子判成健康。
2. **控制传输是带外的，没有定序。** 现在"session 关了"和"这一帧数据"的先后由字节流免费
   保证；搬走之后要么接受竞态，要么自己补一套 quiesce/drain。
3. **同步控制传输把掉线检测拖慢。** 现在是扔进 bulk 流 + 条件变量等 `kSessionAckTimeout`
   = 200 ms；EP0 那条路的超时是 `kControlTimeoutMs` = 1000 ms，且会占住 keepalive 线程。

收益侧则是空的：唯一能拿到的"拒绝立刻 STALL"在实际路径上几乎不触发，因为 session 唯一的
拒绝原因就是没做 EP0 握手，而握手本身在 EP0 上、主机自己知道做没做过。

**性能不是这个决定的判据**——已经测掉了，4 Hz 的 keepalive 频率下对包率的影响测不出来，
见 `USB_OPTIMIZATION_LOG.md` 第 11 节。

## 主机侧板类：一块芯片一个类 [2026-09-05]

`host/include/librmcs/board/rmcs_board_hpm5321.hpp` 现在**同时服务两块 PCB**（单 CAN
`0xA901` 与双 CAN `0xA902`），`rmcs_board_hpm5321_dual_can.hpp` 只剩一个 `using` 别名，
`core/include/librmcs/spec/rmcs_board_hpm5321_dual_can/` 已删除。
...
1. **描述符表现在是"镜像的容量"，不是"某块板的配置"**。`spec::rmcs_board_hpm5321`
   固定两条，真实条数读 `interface().can_count`。头文件里已注明。
2. **越界在运行期拦，不在编译期**。`can_transmit()` 用 `interface().can_count` 兜底，
   在单 CAN 板上发 CAN2 会抛出并指明这块板只有 CAN1，而不是把字段序列化进一个没有固件
   会读的流里。
3. **一个设备可以有多个 PID**。`DeviceScanner::select_device` / `create_transport` /
   `Handler` 现在收 `std::span<const uint16_t>`；单 PID 的板走保留的便利重载，六个其他
   板头一行没改。
...
三个开关都默认 OFF：`CAN_DIAG` 会在 CAN 热路径上加计数器，而 `CAN_DIAG`、
`DIAG_OVER_USB`、`SOF_DIAG` **三个都占用 `DataId::kUart0`**（本板的 UART1 数据口）
把记录当 UART 上行帧发出去（`diag/can_diag.cpp` 的 `write_uart(kUart0, ...)`）。
**三者共用这一条上行通道，不要同时打开**；`SOF_DIAG` 还会额外开 8 kHz 的 SOF 中断。
`TIME_SYNC` 走的是 session 字段而不是 `kUart0`，所以它跟另外三个不冲突；它的 8 kHz
...
> `32000/32000, mismatches=0`。这看起来完全像一个 UART 回归。
>
> **教训：A/B 两个镜像时，先 `grep LIBRMCS_ .../CMakeCache.txt` 对齐编译选项。**
> `cmake --preset` 不会重置已缓存的 `-D` 开关，所以复用旧 build 目录和新建 build
> 目录**默认就不是同一份配置**——本次开发中据此追了一个并不存在的 UART 回归。
