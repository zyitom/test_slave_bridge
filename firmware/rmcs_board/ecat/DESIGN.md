...
| 做法 | 预期 | 实测 | 结论 |
|---|---|---|---|
| CPU governor `powersave` -> `performance` | 降低尾部 | p50/p99/max 全在噪声内（146.3/147.3/159.4 vs 基线 146.8/146.8/155.7） | **对板级 RTT 无效，本行成立**（2026-08-04 用两块板复测，p50 100.6 vs 100.7，min/p99 也全在噪声内）。**但"RT 线程本来就把频率顶住了"这个解释只在忙循环下成立**：核在两次事务之间真的空闲时（1kHz 控制环、间隔 1ms），powersave 会掉到 2.3 GHz，主机路径上值约 10us——只是被 125us 微帧量化吃掉，板级看不见。判据与数据见 [../../../HOST_TUNING.md](../../../HOST_TUNING.md) 1.1 |
...
> **本节的量化结论成立，但由它推出的优化方向（"把格子变小就能少跨格"）已被
> 4.5 节实测推翻 [2026-08-02]。** 本节只往**变慢**的方向扫过（加节拍），
> 4.5 往**变快**的方向扫（缩短帧间隔），发现关系不可反向外推：帧间隔从 26.3us
> 压到 11.9us，RTT 一微秒没降反而升。读本节时请连 4.5 一起读，不要据此去做
> 任何"提高帧率"的优化。
...
### 4.5 主站帧流水线：结论取决于工况，尾部是拦路虎 [实测 2026-08-02]

**结论先行（两句话，缺一不可）：**

1. **在连续流量下，depth 2 是赢的**——p50 −8.6us、p90 −15.8us、p99 −9.5us，三轮复现。
2. **但它的最坏值会炸到 9.7 毫秒**（depth 1 稳定在 0.41-0.44ms），所以**默认仍然是 1**。
   拦路虎是尾部，不是流水线本身没用。

> **本节曾经只写了第 1 段的反面** ——因为最早只用请求/应答工况（`bridge_can_loopback_latency`）
> 测过，而那恰好是流水线最不利的工况：回包时刻与帧到达时刻强相关，"等下一帧"根本
> 不是均匀分布，压缩帧间隔省不到 T/2。**换成灌流工况后结论反转。**
> 教训与 4.6 节同类：**一个工况的实测不能当成系统性质**，尤其当被测量的东西
> （相位等待）的统计分布本身会随工况改变时。

#### 4.5.1 请求/应答工况：depth 1 赢

`bridge_can_loopback_latency ecat enp2s0 3000 7 6`，`chrt -f 80`，`--pmqos` 持住：

做法：`igh.cpp` 的 `RMCS_ECAT_PIPELINE`（默认 1）。IgH 不允许把同一个 datagram
重复排队（只累加 `skip_count`），所以深度必须来自**多个 domain**，每个 domain 在从站
上吃掉一对 FMMU。从站 ESC 寄存器 `0x0004` 读出 **FMMU = 8** [实测]，故深度上限 4。

| depth | 帧率 | 帧间隔 | RTT p50（多轮） | p99 | min | wc err |
|---|---|---|---|---|---|---|
| **1** | 38.0 kHz | 26.3us | **133.4 / 133.2 / 132.8** | 140.3 / 139.8 / 149.4 | 118.8 | 0 |
| 2 | 77.8 kHz | 12.9us | 139.2 / 139.5 / 139.4 | 162.3 / 159.7 / 159.3 | **115.9** | 0 |
| 3 | 84.1 kHz | 11.9us | 140.5 | 154.3 | 123.0 | 1 |
| 4 | 84.3 kHz | 11.9us | 155.5 | 177.2 | 134.5 | 2 |

**这不是实现没写对。** 帧率数字证明流水线完全按设计工作：
84.3 kHz x 148 B x 8 = **99.8 Mbit/s，100BASE-TX 已经跑满**。帧间隔真的从 26.3us
掉到了 11.9us。

唯一变好的是 **min（118.8 -> 115.9）**——多出来的帧确实偶尔能更早接到回包。但这点
收益每一拍都要用额外的从站 PDI 中断和线上占用去换，所以 p50/p99 净亏。

**为什么这个工况下赢不了**：请求/应答里回包出现的时刻**与帧到达时刻强相关**——请求是坐
某一帧来的，回包就在那之后一个近似固定的延迟出现。所以"等下一帧"不是均匀分布，
压缩帧间隔省不到理论上的 T/2，而额外帧的成本照付。[推断，见 4.5.2 的反证]

#### 4.5.2 连续灌流工况：depth 2 赢主体，但尾部炸掉

`ecat_canfd_stress enp2s0 8 5000`（两对 CAN 各 5000 f/s，共 10000 f/s，n=37500/轮），
同样 `chrt -f 80` + `--pmqos`，各 3 轮：

| | depth 1 | depth 2 | 差 |
|---|---|---|---|
| p50 | 159.2 / 160.6 / 159.3 | **150.7 / 150.9 / 151.0** | **-8.6us** |
| p90 | 177.3 / 177.8 / 177.7 | **161.8 / 161.9 / 162.0** | **-15.8us** |
| p99 | 183.9 / 183.6 / 184.3 | **176.2 / 172.9 / 174.3** | **-9.5us** |
| p99.9 | 278.7 / 268.7 / 261.5 | 5281 / 289.7 / 1134 | 不稳定 |
| **max** | **416 / 441 / 410** | **9726 / 2470 / 4560** | **最坏 9.7 毫秒** |

**主体赢得干净且可复现**（p50/p90/p99 三轮全赢，方差极小）。这证实了 4.5.1 的失败
是工况问题：在自发到达、与帧相位无关的上行流量上，压缩帧间隔**真的**能省掉
"等下一帧来取件"的时间，这正是本系统真实工况（4 路 CAN x 7 电机 @1kHz 自发反馈）
的形状。

**但尾部不能接受**：最坏 9.7ms = **1kHz 控制环连丢 10 拍**，且随机（p99.9 三轮
290 / 1134 / 5281）。depth 1 的 max 稳定在 0.41-0.44ms。

**尾部的形状是 go-back-N 重传的签名**：偶发 chunk 丢失 -> 接收端扣住 ack ->
`kGoBackStallFrames`(4) 帧后窗口停摆重放。**但这一条目前只是推断——
`PdStreamEndpoint` 的 go-back 路径没有任何计数器，触发了在主机侧看不见。**
[推断，未证实]

**处置**：`RMCS_ECAT_PIPELINE` **默认保持 1**，但这是被尾部挡住，**不是因为流水线没用**。
要解锁 depth 2 的那 8-16us，前置工作是：

1. 给 `PdStreamEndpoint` 加 go-back 计数器，与 wc error 一起打进 frame-rate 日志
   ——**没有它，尾部归因只能靠猜**；
2. 确认尾部确实来自 go-back 后，再决定是调 `kGoBackStallFrames`、加深
   `kPdWindow`，还是消除丢包本身。

另外深度还被 ARQ 窗口 `kPdWindow == 2` 二次限制：超过 2 帧在飞时端点已经没有 credit
去装新 chunk，多出来的 channel 只能重画最新那个——这也是 depth 3/4 在 4.5.1 里
持续变差的结构原因。

**与 USB 的对照仍然成立**：`host/src/transport/usb/usb.cpp` 早就在做同一件事
（16 个预提交 RX transfer），注释写的是"4-deep 藏不住 submit/callback 往返，
把 **throughput** 压到远低于链路速率"。**流水线的第一性作用是填链路**；
它在 4.5.2 里顺带降了延迟，是因为这里的延迟里确实含有一段"等发车"的相位等待。
...
1. **上位机运行环境** —— **已逐项核查并脚本化为 `./host-tuning.sh`（仓库根目录）**。
   ...
   | CPU governor `performance` | 已设 | 板级 RTT 上无效（见 3.6）；**主机路径上约 10us，2026-08-04 实测** | 否 |
