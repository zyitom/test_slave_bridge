 ...
 * (samples/ethercat/port/hpm_ecat_hw.c) and by the project's read-only EEPROM
 * copy (hpm_ecat_e2p_emulation.c). Kept identical to
 * ../../core0/src/ecat_config.h: the core swap moves which core runs this code,
 * not which peripherals or flash geometry it uses.
 */
...
/* Check Product Code / Revision in the emulated EEPROM. On core1 a mismatch is
 * reported, never repaired -- the refresh needs flash writes, which belong to
 * core0 (migration step 3). See hpm_ecat_e2p_emulation.c. */
 ...
 * reboot. Note this now reboots BOTH cores from core1's ISR, which is the
 * intended behaviour but a change of blast radius worth knowing about. */
