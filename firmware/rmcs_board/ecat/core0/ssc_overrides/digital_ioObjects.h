 ...
 *
 * Layout: one 48-byte stream chunk per direction (see common/pd_stream.hpp),
 * described to CoE as arrays of 12 UNSIGNED32 because a single PDO-mapping
 * entry cannot exceed 255 bits (8-bit length field, ETG.1000.6). The mapping
 * is fixed: 0x1A00 maps 0x6000:01..:0C (inputs, slave -> master), 0x1600 maps
 * 0x7010:01..:0C (outputs, master -> slave). Sizes must agree with
