...
**这个目录现在同时服务两块板**：单 CAN 版（只有 MCAN0，RGB 灯在 PA29/30/31，无 CAN 指示
灯）和双 CAN 版（MCAN0 + MCAN3，RGB 灯在 PA26/27/28，两颗 CAN 指示灯在 PB14/PB15）。
两块板的 CAN 控制器**都跑 CAN-FD**，变体只决定启用几路。
...
| 主机侧端点 | 板上资源 | 说明 |
| ------------- | -------------- | ------------------------------ |
| `DataId::kCan0` | MCAN0（CAN-FD，1M 仲裁 / 5M 数据） | DM（达妙）电机总线 |
...
此前 `can_port()` 会在单 CAN 版上把 `.mode` 覆盖成 `kClassic`，理由只是"保持与出厂
固件逐字节一致"。这个配置是**单向亏**的——classic 不是超集，对端发来的 FD 帧一帧都
收不到。当时的实测（单 CAN 版与 mc02 的 FDCAN3 对接，mc02 常驻 FD+BRS）：
...
`[实测 2026-08-05]`。**移除覆盖后这张表不再描述本仓库的行为**，但那个签名仍值得记住：
一个方向通、另一个方向零帧，看起来很像 [../../AGENTS.md](../../AGENTS.md) 里记的采样点
不一致签名，成因却完全不同（那是采样点，这是控制器不支持 FD 格式），且总线上不会有错误
计数。日后再遇到"classic 通、FD 单向不通、错误计数为 0"，先查**对端**的控制器模式。

#### 尚未验证：单 CAN 版 PCB 的收发器能否跑 5 Mbit 数据段 [未实测]

开 FD 会同时配上 `baudrate_fd = 5'000'000`（`app/src/can/can.hpp` 的 `Can` 构造函数）。
双 CAN-FD 版的收发器已知可用；**单 CAN 版的收发器型号尚未核对，也没有在实板上测过 FD**。
上板前需要确认两条：

1. 该板收发器的数据段速率上限 >= 5 Mbit；
2. 与实际电机（DJI / 达妙 / 瓴控，全部 87.5% 采样点）双向 FD 收发通过。

在这两条确认之前，单 CAN 版开 FD 属于**未验证的改动**——它在逻辑上严格更强，但电气上
没有被证实过。

注意 `host/examples/mixed_board_test` 无法自动识别板子的 FD 能力，`RMCS_CAN_FD=1` 时
照样打印 "CAN-FD"。开关名是 **`RMCS_CAN_FD`**，写成 `RMCS_FDCAN` 不会报错，只会静默按
classic 跑，测出来的"FD 通过"是假的。
