...
## 双板对打实测：延迟与吞吐上限 [实测 2026-08-04]

两块本板互连（A.CAN0<->B.CAN0、A.CAN1<->B.CAN1、UART0<->UART0，两端各一个 120 欧），
`host/examples/dual_board_test`，固件 `v3.2.2-0.dev.47.g493b564`。每帧都跨越真实总线、
经过两个独立控制器和两个独立时钟——这是单板自回环测不到的。

### 延迟（CAN-FD 8 字节，2000 样本）

| 采样间隔 | min | p50 | p99 | max |
|---|---|---|---|---|
| 0（紧凑 ping-pong，约 10kHz） | 88.2 | **100.5** | 104.6 | 123.6 |
| 1000us（1kHz 控制环占空比） | 77.7 | **101.7** | 124.4 | 134.0 |

两条总线对称（CAN1 的 p50 为 100.7），2000/2000 送达、0 超时 0 错帧。
经典 CAN 2.0 同条件 p50 为 **166.5**——与 FD 差 **66us**，正是两者线上时间之差。

> **测主机调优时采样间隔必须设成 1000us**，否则紧凑循环会把 CPU 频率自己顶上去，
> 测不出 governor 一类的差别。判据见 [../../../../HOST_TUNING.md](../../../../HOST_TUNING.md) 1.1/1.2。

### 吞吐（CAN-FD 8 字节，双总线同时，每总线各计）

| 请求速率/总线 | 实际送达 | 丢失 | 判定 |
|---|---|---|---|
| 4000 f/s | 4000 | 0.0000% | PASS |
| 8000 f/s | 8000 | 0.0000% | PASS |
| 16000 f/s | 16000 | 0.0000% | PASS |
| **19000 f/s** | **19000** | **0.0000%** | **PASS（已验证的最高无损速率）** |
| 32000 f/s | **约 19870 / 20270** | 37.9% / 36.7% | FAIL |

**瓶颈是 CAN 线，不是板子也不是 USB。** 32000 f/s 那一行实际只推出约 20000 f/s，正是
8 字节 CAN-FD 帧（仲裁 1M + 数据 5M）约 50us 帧时的线速上限。**且 `corrupt=0`、
错的全是顺序缺口**——超载时是干净地丢帧，不是把帧弄坏，这是正确的失效形态。

双总线合计 38000 f/s 无损（19000 x 2），累计 57 万帧 0 丢失 0 缺口。

### UART

921600 波特，2000 轮 x 64 字节 = 128000 字节，**0 失配**，实测 57.1 kB/s
（线速 92.2 kB/s，差值是逐轮握手的往返开销，不是丢字节）。
