...
int main(int argc, char** argv) {
    ...
    std::fprintf(stderr, "cycles %llu  cycle rate %.1f kHz  timeouts %llu\n",
    ...
    std::fprintf(stderr, "loopback depth (cycles): p50 %.0f  p90 %.0f  max %u  (compare ARQ ~4)\n",
                 pct(depth_cycles, 0.50), pct(depth_cycles, 0.90), depth_cycles.back());
    std::fprintf(stderr, "round-trip us: p50 %.1f  p90 %.1f  p99 %.1f  max %.1f\n",
                 pct(rtt_us, 0.50), pct(rtt_us, 0.90), pct(rtt_us, 0.99),
                 rtt_us.empty() ? 0.0 : rtt_us.back());
    std::fprintf(stderr,
                 "reliability cost of dropping ARQ: %llu drops/dups + %llu reorders over %llu "
                 "echoes (%.3f%%)\n",
                 static_cast<unsigned long long>(drops_or_dups),
