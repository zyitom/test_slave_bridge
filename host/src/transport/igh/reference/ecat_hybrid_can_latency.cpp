// Queue-free CAN-FD loopback latency over the HYBRID fixed-PDO firmware
// (RMCS_ECAT_HYBRID_PD): the 352-byte PDO is a 336-byte cyclic CAN region
// (seven 12-byte slots per bus, latest-wins, no ARQ) followed by a 16-byte
// pd_stream chunk. The stream performs only the production session handshake
// and keepalive; measured CAN data stays in the mailbox region. It is the
// hybrid twin of ecat_native_can_latency.cpp and is
// measured identically (free-running cycle thread + a busy-waiting sender, one
// frame in flight) so the RTT is directly comparable to that tool's numbers.
//
...
// build); run from the repo root, needs the IgH master running. OUT is the
// output directory (the session scratchpad was used for verification):
//   g++ -O2 -std=c++20 -I/usr/local/include -Icore/include
//     host/src/transport/igh/reference/ecat_hybrid_can_latency.cpp -o
//     "$OUT/ecat_hybrid_can_latency" -L/usr/local/lib -lethercat -lpthread -lrt
//   RMCS_ECAT_BACKEND is irrelevant here; the master is chosen via ethercat.conf.
//   sudo ./ecat_hybrid_can_latency [samples] [cycle_core] [main_core]
...
// Slave identity, identical to host/src/transport/igh/igh.cpp.
...
// Hybrid mapping: each direction maps an 84 x UNSIGNED32 fixed array followed
// by a 4 x UNSIGNED32 stream array (see librmcs/ecat/hybrid_pd.hpp).
constexpr uint16_t kOutMailboxIndex = 0x7000;
constexpr uint16_t kOutStreamIndex = 0x7010;
constexpr uint16_t kInMailboxIndex = 0x6000;
constexpr uint16_t kInStreamIndex = 0x6010;
constexpr unsigned kMailboxEntryCount = 84; // 336-byte fixed region
constexpr unsigned kStreamEntryCount = 4;   // 16-byte stream chunk
constexpr unsigned kPdoEntryCount = kMailboxEntryCount + kStreamEntryCount; // 88
...
// sender. Downlink is published under a generation counter; uplink is published
// with a seqlock so the sender reads a torn-free snapshot. Only one downlink
// slot is used; all seven rotating uplink slots of CAN1 are scanned.
struct Shared {
    std::atomic<bool> stop{false};

    std::atomic<uint32_t> down_gen{0};
    uint8_t down_image[native::kHybridPdSize] = {};

    std::atomic<uint32_t> up_gen{0};
    uint8_t up_image[native::kHybridPdSize] = {};
...
int main(int argc, char** argv) {
    ...
    // 88 entries per direction: 84 fixed u32 (0x7000 / 0x6000) then 4 stream
    // u32 (0x7010 / 0x6010).
    ec_pdo_entry_info_t out_entries[kPdoEntryCount];
    ec_pdo_entry_info_t in_entries[kPdoEntryCount];
    for (unsigned i = 0; i < kMailboxEntryCount; ++i) {
        out_entries[i] = {kOutMailboxIndex, static_cast<uint8_t>(i + 1), 32};
        in_entries[i] = {kInMailboxIndex, static_cast<uint8_t>(i + 1), 32};
    }
    for (unsigned i = 0; i < kStreamEntryCount; ++i) {
        out_entries[kMailboxEntryCount + i] = {kOutStreamIndex, static_cast<uint8_t>(i + 1), 32};
        in_entries[kMailboxEntryCount + i] = {kInStreamIndex, static_cast<uint8_t>(i + 1), 32};
    }
    ...
    // Register the first mailbox entry of each direction; the mailbox region
    // begins at that domain offset (offset 0 within the 352-byte image, since
    // 0x7000 / 0x6000 map first -- see hybrid_pd.hpp).
    const int off_out = ecrt_slave_config_reg_pdo_entry(sc, kOutMailboxIndex, 1, domain, nullptr);
    const int off_in = ecrt_slave_config_reg_pdo_entry(sc, kInMailboxIndex, 1, domain, nullptr);
    ...
    std::memset(outputs, 0, native::kHybridPdSize);
    reference::HybridSession session;

    auto exchange_once = [&]() {
        session.prepare_output(outputs, std::chrono::steady_clock::now());
        ...
        if (ds.wc_state == EC_WC_COMPLETE)
            session.consume_input(inputs, std::chrono::steady_clock::now());
        return ds;
    };
    ...
    session.begin();
    const auto session_deadline = std::chrono::steady_clock::now() + kSessionTimeout;
    ...
    printf("hybrid CAN mailbox link up: vendor 0x%08X product 0x%08X\n", kVendorId, kProductCode);
    ...
    std::thread cycle_thread{[&]() {
        configure_thread(cycle_core, 80, "hybrid-cyc");
        uint32_t seen_down_gen = 0;
        uint8_t local_down[native::kHybridPdSize] = {};
        uint8_t candidate_down[native::kHybridPdSize];
        while (!shared.stop.load(std::memory_order_relaxed)) {
            const uint32_t g1 = shared.down_gen.load(std::memory_order_acquire);
            if ((g1 & 1U) == 0 && g1 != seen_down_gen) {
                std::memcpy(candidate_down, shared.down_image, native::kHybridPdSize);
                const uint32_t g2 = shared.down_gen.load(std::memory_order_acquire);
                if (g1 == g2) {
                    std::memcpy(local_down, candidate_down, native::kHybridPdSize);
                    seen_down_gen = g2;
                }
            }
            std::memcpy(outputs, local_down, native::kHybridPdSize);
            ...
            std::memcpy(shared.up_image, inputs, native::kHybridPdSize);
            shared.up_gen.store(seq + 2, std::memory_order_release); // even: stable
        }
    }};

    configure_thread(main_core, 70, "hybrid-main");
    ...
    for (uint32_t sequence = 0; sequence < samples + kWarmupSamples; ++sequence) {
        ...
        // Stage the downlink mailbox for bus 0 (mailbox region at image offset 0).
        mailbox_seq = mailbox_seq == 255 ? 1 : static_cast<uint8_t>(mailbox_seq + 1);
        uint8_t image[native::kHybridPdSize] = {};
        uint8_t* mb = image + native::hybrid_mailbox_offset(kTxBus, 0);
        ...
        shared.down_gen.store(g + 1, std::memory_order_release); // odd: writing
        std::memcpy(shared.down_image, image, native::kHybridPdSize);
        shared.down_gen.store(g + 2, std::memory_order_release); // even: stable
        ...
        while (Clock::now() < deadline) {
            ...
            if (g1 & 1u)
                continue;
            uint8_t snapshot[native::kHybridPdSize];
            std::memcpy(snapshot, shared.up_image, native::kHybridPdSize);
            const uint32_t g2 = shared.up_gen.load(std::memory_order_acquire);
            if (g1 != g2)
                continue;
            for (std::size_t slot = 0; slot < native::kHybridSlotsPerBus; ++slot) {
                const uint8_t* rx = snapshot + native::hybrid_mailbox_offset(kRxBus, slot);
    ...
    if (rtts_us.empty()) {
        fprintf(stderr, "no valid loopback frames (check CAN0<->CAN1 wiring + hybrid firmware)\n");
        ecrt_master_deactivate(master);
        ecrt_release_master(master);
        return 2;
    }
    ...
    for (double v : rtts_us)
        sum += v;
    printf(
        "hybrid CAN: %zu round trips, timeout=%u, %llu cycles, %llu wc errors\n", rtts_us.size(),
